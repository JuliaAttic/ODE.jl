        ...
        for ts in testsets
            ...
            for points = [:specified, :all]
                if ts[:isscalar]
                    ...
                    y0scal = y0[1]
                    # with jacobian
                    tj,yj = ODE.solve(Fscal,y0scal,tout,
                                    solver = stepper,
                                    points = points,
                                    initstep = h0,
                                    J! = jac!)
                    ...
                    # without jacobian
                    t,y   = ODE.solve(Fscal,y0scal,tout,
                                    solver = stepper,
                                    points = points,
                                    initstep = h0)
                ...
                # # with jacobian
                # tj,yj = ODE.solve(F,reshape(y0,length(y0),1,1),tout,
                #                 solver = stepper,
                #                 points = points,
                #                 initstep = h0,
                #                 J! = jac!)
                # @test_approx_eq_eps hcat(yj...) hcat(map(sol,tj)...) tol
                # # without jacobian
                # t,y   = ODE.solve(F,reshape(y0,length(y0),1,1),tout,
                #                 solver = stepper,
                #                 points = points,
                #                 initstep = h0)
                # @test_approx_eq_eps hcat(y...)  hcat(map(sol,t)...) tol
                ...
                prob   = ODE.Problem(equation,stepper;opts...)

                for (t,y) in prob
                    @test_approx_eq_eps y sol(t) tol
                end

                for (t,y) in ODE.Problem(equation,ODE.DenseOutput{stepper}; opts...)
                    @test_approx_eq_eps y sol(t) tol
                end
