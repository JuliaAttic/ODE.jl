# TODO: currently the prototype has terrible performance on this!
...
## ode23s
tout, yout = ODE.ode23s(fn_ex, ic, tspan0, jacobian=jac_ex)
print("ODE.ode23s: ")
gc()
@time tout, yout = ODE.ode23s(fn_ex, ic, tspan, jacobian=jac_ex)
@show norm(abs(yout[end][refsolinds]-refsol)./refsol, Inf)

# # ## DASSL
# t,ydassl = DASSL.dasslSolve(fn, ic, tspan0, jacobian=jac)
# @time t,ydassl = DASSL.dasslSolve(fn, ic, tspan, jacobian=jac)
# @show norm(abs(ydassl[end][refsolinds]-refsol)./refsol, Inf)

## New ODE
# No jac
solver = [Rosenbrock23s{:v1}, Rosenbrock23s{:v2}][2]
diffeq = ExplicitODE(tuple(tspan0...), ic, fn_ex!)
opts = get_solver_defopts(diffeq,solver)
opts[:reltol] = 1e-6
odep = Problem(diffeq,solver,opts)
print("solve:      ")
tout, yout = solve(odep)

diffeq = ExplicitODE(tuple(tspan...), ic, fn_ex!)
odep = Problem(diffeq,solver,opts)
gc()
@time tout, yout = solve(odep)
@show norm(abs(yout[refsolinds,end]-refsol)./refsol, Inf)

tout, yout = ode23s(fn_ex, ic, tspan0)
print("ode23s:     ")
gc()
@time tout, yout = ode23s(fn_ex, ic, tspan)
@show norm(abs(yout[refsolinds,end]-refsol)./refsol, Inf)
